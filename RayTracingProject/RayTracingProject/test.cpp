...
int main() {
	...
	Material* mat1 = new Material(Vec3(0, 255, 0), 0.2, 0.5, 0.8);
	Material* mat2 = new Material(Vec3(255, 0, 0), 0.2, 0.7, 0);
	Light light = Light(Vec3(), 1, Vec3(150, 150, -50));
	...
	for (int i = 0; i < WIDTH; i++) {
		for (int j = 0; j < HEIGHT; j++) {
			...
			for (int k = 0; k < myScene.Objects.size(); k++) {
				...
				if (IntersectionChecker().checkIntersection(view, obj, &output, &dist)) {
			...
			if (firstInSight) {
				...
				for (int z = 0; z < myScene.lights.size(); z++) {
					...
					Vec3 h = (lightVec + viewVec) / (lightVec + viewVec).getLength();
					float NL = normal * lightVec;

					if (l.intensity > ia)
						ia = l.intensity;

					Ray lightRay = {outputInfos, lightVec};
					bool hidden = false;

					Vec3 tmp1;
					float tmp2;

					for (int w = 0; w < myScene.Objects.size() && !hidden; w++)
						if (myScene.Objects[w] != firstInSight)
							if (IntersectionChecker().checkIntersection(lightRay, myScene.Objects[w], &tmp1, &tmp2))
								hidden = tmp2 > 0.01;

					if (normal * lightVec > 0.01 && !hidden) {
				...
				Vec3 pix = clampColor(mat.color * (mat.ambiance * ia) + mat.color * (mat.diffuse * id) + Vec3(255, 255, 255) * (mat.specular * is));
				color.rgbRed = pix.x;
